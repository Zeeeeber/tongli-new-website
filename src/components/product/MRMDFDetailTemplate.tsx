...
interface MRMDFDetailTemplateProps {
...
export function MRMDFDetailTemplate({
  product,
  slug,
}: MRMDFDetailTemplateProps) {
      ...
      <div className="bg-[#F7F3EC] py-4">
        <div className="container mx-auto px-6">
          <div className="flex items-center gap-2 text-sm text-[#6b7280]">
            ...
            <Link href="/products/supporting-boards" className="hover:text-[#0F6B3A]"><T>{"MR MDF"}</T></Link>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {relatedProducts.length > 0 ? (
              relatedProducts.map((relatedProduct) => {
                ...
                const relatedLabel = (relatedProduct as { subCategory?: string }).subCategory || relatedProduct.specs.veneerSpecies || relatedProduct.tags[0] || "MR MDF";
              ...
              placeholderRelatedProducts.map((item) => (
                ...
                >
                  ...
                  <div className="p-4">
                    <span className="inline-flex items-center rounded-full bg-[#F7F3EC] px-3 py-1 text-[11px] font-medium text-[#0F6B3A]"><T>{"MR MDF"}</T></span>
                    <h3 className="font-semibold text-[#1F2621] mt-3 mb-2 line-clamp-2"><T>{"Related Product"}</T></h3>
                    <p className="text-sm text-[#6b7280] line-clamp-3"><T>{"More MR MDF products will appear here."}</T></p>
                  </div>
                </div>
              ))
            )}
...
export default MRMDFDetailTemplate;
