...
import { engineeredWoodVeneerProducts, type EngineeredWoodVeneerProduct } from "@/data/products/engineered-wood-veneer-products";

interface EngineeredWoodVeneerDetailTemplateProps {
  product: EngineeredWoodVeneerProduct;
  slug: string;
}
...
const defaultQualityPackaging = [
  "Pattern consistency across all sheets",
  "Color stable and batch-uniform",
  "Thickness tolerance within ±0.05mm",
  "Flatness inspection and quality control",
...
export function EngineeredWoodVeneerDetailTemplate({
  product,
  slug,
}: EngineeredWoodVeneerDetailTemplateProps) {
  ...
  const relatedProducts = useMemo(() => {
    ...
    return engineeredWoodVeneerProducts
      .filter((candidate) => candidate.slug !== slug)
      .map((candidate) => {
        let score = 0;

        if (candidate.specs.pattern === product.specs.pattern) {
          score += 4;
        }

        if (candidate.specs.grainStyle === product.specs.grainStyle) {
          score += 3;
        }
  ...
  }, [product.specs.grainStyle, product.specs.pattern, product.tags, slug]);
  ...
  return (
    <>
      ...
      <div className="bg-[#F7F3EC] py-4">
        <div className="container mx-auto px-6">
          <div className="flex items-center gap-2 text-sm text-[#6b7280]">
            ...
            <Link href="/products/engineered-wood-veneer" className="hover:text-[#0F6B3A]"><T>{"Engineered Wood Veneer"}</T></Link>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
              ...
              <div className="bg-[#FDFBF7] rounded-2xl border border-[#E5E1D8] overflow-hidden sticky top-24">
                ...
                <nav className="py-2">
                  {[
                    ...
                    { name: "Engineered Wood Veneer", href: "/products/engineered-wood-veneer", active: true },
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {relatedProducts.length > 0 ? (
              relatedProducts.map((relatedProduct) => {
                ...
                const relatedLabel = relatedProduct.specs.pattern || relatedProduct.tags[0] || "Engineered Wood Veneer";

                return (
                  <Link
                    key={relatedProduct.slug}
                    href={`/products/engineered-wood-veneer/${relatedProduct.slug}`}
              ...
              placeholderRelatedProducts.map((item) => (
                ...
                >
                  ...
                  <div className="p-4">
                    <span className="inline-flex items-center rounded-full bg-[#F7F3EC] px-3 py-1 text-[11px] font-medium text-[#0F6B3A]"><T>{"Engineered Wood Veneer"}</T></span>
                    <h3 className="font-semibold text-[#1F2621] mt-3 mb-2 line-clamp-2"><T>{"Related Product"}</T></h3>
                    <p className="text-sm text-[#6b7280] line-clamp-3"><T>{"More engineered wood veneer products will appear here."}</T></p>
                  </div>
                </div>
              ))
            )}
              ...
              <table className="w-full min-w-[600px]">
                <tbody>
                  {[
                    ...
                    { label: "Product Name", value: "Reconstituted Veneer, Engineered Veneer, Artificial Veneer, Man-Made Veneer, Composite Wood Veneer, Recon Wood Veneer, Recomposed Veneer, Synthetic Wood Veneer" },
                    { label: "Pattern", value: product.specs.pattern },
                    { label: "Length", value: "2.5-3.2M" },
                    { label: "Width", value: "Mainly 650mm" },
                    { label: "Thickness", value: product.specs.veneerThickness },
                    ...
                    { label: "Kinds Of Export Packing", value: "Palletized Packaging, Stretch Film Wrapping, Cardboard Box Packaging" },
      ...
      {/* Product Detail Content - 8 Sections (alternating: odd=image-left/text-right, even=text-left/image-right) */}
      <div>
        {/* 01. Banner - image left, text right */}
        <section className="py-10 sm:py-16 bg-white">
          <div className="container mx-auto px-4 sm:px-6">
            <div className="flex flex-col md:flex-row items-center gap-8 md:gap-16">
              <div className="w-full md:w-1/2">
                <div className="w-full rounded-2xl overflow-hidden">
                  <Image src="/images/products/products_engineered wood veneer_detail page/1.Banner_engineered wood veneer manufacturer.jpg" alt="Engineered Wood Veneer Manufacturer" width={800} height={600} className="w-full h-auto" unoptimized />
                </div>
              </div>
              <div className="flex-1">
                ...
                <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-[#1F2621] mb-3"><T>{"Engineered Wood Veneer Manufacturer"}</T></h2>
                <p className="text-sm text-[#8B5E3C] mb-4 sm:mb-6 italic"><T>{"Supplying reconstituted wood veneer sheets with consistent patterns for furniture, doors, panels and interior projects."}</T></p>
                <p className="text-[#6b7280] text-sm leading-relaxed mb-4 sm:mb-6"><T>{"Tongli Timber supplies engineered wood veneer (reconstituted veneer) with 300+ consistent patterns, stable colors, and uniform textures. From pattern selection and color matching to backing options, packing and export delivery, we help furniture manufacturers, door factories, panel producers and interior project buyers source stable and beautiful veneer materials for production."}</T></p>
                <div className="grid grid-cols-1 gap-2 sm:gap-3 mb-6 sm:mb-8">
                  {["Engineered wood veneer sheets with consistent patterns", "300+ patterns and growing pattern library", "Color-stable and batch-uniform production", "Custom backing options: loose, fleece, paper", "Factory direct supply with export experience"].map((point) => (
                    <div key={point} className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-full bg-[#0F6B3A]/10 flex items-center justify-center flex-shrink-0"><svg className="w-3 h-3 text-[#0F6B3A]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg></div>
        ...
        {/* 02. Real Shots - text left, image right */}
        <section className="py-10 sm:py-16 bg-[#FDFBF7]">
          <div className="container mx-auto px-4 sm:px-6">
            <div className="flex flex-col md:flex-row items-center gap-8 md:gap-16">
              <div className="flex-1">
                <p className="text-[10px] sm:text-xs font-semibold text-[#0F6B3A] uppercase tracking-wider mb-2 sm:mb-3"><T>{"Real Shots"}</T></p>
                <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-[#1F2621] mb-3"><T>{"Engineered Wood Veneer Sheets for Selection"}</T></h2>
                <p className="text-sm text-[#8B5E3C] mb-4 sm:mb-6 italic"><T>{"Consistent grain, stable color and real material details before order confirmation."}</T></p>
                <p className="text-[#6b7280] text-sm leading-relaxed mb-4 sm:mb-6"><T>{"Engineered wood veneer offers consistent grain patterns and stable colors across all sheets. Our real product photos help customers check veneer appearance, pattern consistency, sheet quality and color tone before placing orders. Whether you need oak, walnut, ash, teak or other reconstituted patterns, we can provide sample photos, catalog references and physical samples for approval."}</T></p>
              ...
              <div className="w-full md:w-1/2">
                <div className="w-full rounded-2xl overflow-hidden">
                  <Image src="/images/products/products_engineered wood veneer_detail page/2.engineered wood veneer real shots.jpg" alt="Engineered Wood Veneer Real Shots" width={800} height={600} className="w-full h-auto" unoptimized />
                </div>
              </div>
            </div>
          </div>
        ...
        {/* 03. Application - image left, text right */}
        <section className="py-10 sm:py-16 bg-white">
          <div className="container mx-auto px-4 sm:px-6">
            <div className="flex flex-col md:flex-row items-center gap-8 md:gap-16">
              <div className="w-full md:w-1/2">
                <div className="w-full rounded-2xl overflow-hidden">
                  <Image src="/images/products/products_engineered wood veneer_detail page/3.engineered wood veneer application.jpg" alt="Engineered Wood Veneer Application" width={800} height={600} className="w-full h-auto" unoptimized />
                </div>
              </div>
              <div className="flex-1">
                <p className="text-[10px] sm:text-xs font-semibold text-[#0F6B3A] uppercase tracking-wider mb-2 sm:mb-3"><T>{"Applications"}</T></p>
                <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-[#1F2621] mb-3"><T>{"Engineered Wood Veneer for High-end Interior Surfaces"}</T></h2>
                <p className="text-sm text-[#8B5E3C] mb-4 sm:mb-6 italic"><T>{"A flexible decorative material with consistent patterns for furniture, wall panels, doors and custom interiors."}</T></p>
                <p className="text-[#6b7280] text-sm leading-relaxed mb-4 sm:mb-6"><T>{"Engineered wood veneer is widely used in furniture, wooden doors, cabinets, wardrobes, wall cladding, hotel interiors and full-house customization projects. It offers consistent patterns while maintaining the aesthetic of natural wood, with better material efficiency and batch-to-batch uniformity. With different patterns and backing options, customers can create warm, elegant and customized interior surfaces."}</T></p>
                <div className="grid grid-cols-1 gap-2 sm:gap-3">
        ...
        {/* 05. Company Profile - text left, image right */}
        <section className="py-10 sm:py-16 bg-white">
          <div className="container mx-auto px-4 sm:px-6">
            <div className="flex flex-col md:flex-row items-center gap-8 md:gap-16">
              <div className="flex-1">
                ...
                <p className="text-sm text-[#8B5E3C] mb-4 sm:mb-6 italic"><T>{"Experienced manufacturer for engineered veneer, natural veneer, veneer panels and decorative wood materials."}</T></p>
                <p className="text-[#6b7280] text-sm leading-relaxed mb-4 sm:mb-6"><T>{"Dongguan Tongli Timber Products Co., Ltd. was established in 1999 and specializes in engineered wood veneer, natural wood veneer, veneer plywood, fancy plywood, UV coated veneer panels and 3D wood panels. With years of production experience and export service, we support global customers with stable material supply, customized processing and professional communication."}</T></p>
                <div className="grid grid-cols-1 gap-2 sm:gap-3">
                  {["Established in 1999", "Experienced in engineered and natural veneer production", "Factory facilities for veneer processing and lamination", "Support for samples, customization and bulk orders", "Serving furniture, door, panel and interior project customers worldwide"].map((item) => (
                    <div key={item} className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-full bg-[#0F6B3A]/10 flex items-center justify-center flex-shrink-0"><svg className="w-3 h-3 text-[#0F6B3A]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg></div>
                      <span className="text-sm text-[#6b7280]"><T>{item}</T></span>
                    </div>
              ...
              <div className="w-full md:w-1/2">
                <div className="w-full rounded-2xl overflow-hidden">
                  <Image src="/images/products/products_engineered wood veneer_detail page/5.engineered wood veneer_Company Profile.jpg" alt="Company Profile" width={800} height={600} className="w-full h-auto" unoptimized />
                </div>
              </div>
            </div>
          </div>
        ...
        {/* 06. Customer Feedback - image left, text right */}
        <section className="py-10 sm:py-16 bg-[#FDFBF7]">
          <div className="container mx-auto px-4 sm:px-6">
            <div className="flex flex-col md:flex-row items-center gap-8 md:gap-16">
              <div className="w-full md:w-1/2">
                <div className="w-full rounded-2xl overflow-hidden">
                  <Image src="/images/products/products_engineered wood veneer_detail page/6.engineered wood veneer_custom feedback.jpg" alt="Customer Feedback" width={800} height={600} className="w-full h-auto" unoptimized />
                </div>
              </div>
              <div className="flex-1">
                ...
                <p className="text-[#6b7280] text-sm leading-relaxed mb-4 sm:mb-6"><T>{"Many customers choose us again because of stable veneer quality, consistent patterns, clear communication and practical support during sourcing. From sample checking to bulk order delivery, we help buyers confirm patterns, veneer thickness, backing options, edge banding compatibility and packaging requirements."}</T></p>
                <div className="grid grid-cols-1 gap-2 sm:gap-3">
                  {["Consistent pattern quality batch to batch", "Accurate sample confirmation", "Matched veneer and edge banding options", "Clear order communication", "Reliable packaging and delivery", "Support for repeat orders and long-term cooperation"].map((item) => (
                    <div key={item} className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-full bg-[#0F6B3A]/10 flex items-center justify-center flex-shrink-0"><svg className="w-3 h-3 text-[#0F6B3A]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg></div>
                      <span className="text-sm text-[#6b7280]"><T>{item}</T></span>
                    </div>
        ...
        {/* 07. Certifications - text left, image right */}
        <section className="py-10 sm:py-16 bg-white">
          <div className="container mx-auto px-4 sm:px-6">
            <div className="flex flex-col md:flex-row items-center gap-8 md:gap-16">
              <div className="flex-1">
              ...
              <div className="w-full md:w-1/2">
                <div className="w-full rounded-2xl overflow-hidden">
                  <Image src="/images/products/products_engineered wood veneer_detail page/7.engineered wood veneer_Certification.jpg" alt="Certifications" width={800} height={600} className="w-full h-auto" unoptimized />
                </div>
              </div>
            </div>
          </div>
        ...
        {/* 08. Packaging - image left, text right */}
        <section className="py-10 sm:py-16 bg-[#FDFBF7]">
          <div className="container mx-auto px-4 sm:px-6">
            <div className="flex flex-col md:flex-row items-center gap-8 md:gap-16">
              <div className="w-full md:w-1/2">
                <div className="w-full rounded-2xl overflow-hidden">
                  <Image src="/images/products/products_engineered wood veneer_detail page/8.engineered wood veneer packaging.jpg" alt="Packaging Options" width={800} height={600} className="w-full h-auto" unoptimized />
                </div>
              </div>
              <div className="flex-1">
        ...
        <div className="container mx-auto px-4 sm:px-6">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-8 sm:mb-12">
              ...
              <p className="text-[#6b7280] mt-3 sm:mt-4 text-xs sm:text-sm"><T>{"Everything you need to know about our engineered wood veneer"}</T></p>
            </div>
...
export default EngineeredWoodVeneerDetailTemplate;
